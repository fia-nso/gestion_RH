...
app.post(
  ...
  async ({ body, headers }) => {
    ...
    const isEmployer = currentRoles.includes("employer");
    ...
    if (!isAdmin) {
      if (isEmployer && (roles.length !== 1 || roles[0] !== "employer")) {
        return {
          success: false,
          error: "Employers can only create users with the employer role",
        };
      }
    }
  ...
  },
  {
    body: t.Object({
      ...
      password: t.String(),
      name: t.String(),
      roles: t.Array(t.String()), // e.g., ['admin']
    }),
  }
);
...
app.group("/employers", (app) =>
  app
    ...
    //     { name, email, phone, photo_url: photoUrl },
    //   ]).select().single();
